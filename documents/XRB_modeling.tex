...
\end{equation}
To obtain $M_1'$ from the other quantities, we need to use Equation \ref{eq:SN_v_sys}. Here we make the substitution:
\begin{equation}
x = v_r (\alpha - \beta), \label{eq:x_substitution}
\end{equation}
so that all the variables containing $M_1'$ are now consolidated into $x$. Solving for $x$, we find:
\begin{equation}
x = -\beta v_k \cos\ \theta \pm \sqrt{v_{\rm sys}^2 - \beta^2 v_k^2 \sin^2 \theta}.
\end{equation}
This equation has two constraints:
\begin{eqnarray}
v_{\rm sys}^2 &>& \beta^2 v_k^2 \sin^2\ \theta \\
\beta v_k \cos\ \theta &<& \pm \sqrt{v_{\rm sys}^2 - \beta^2 v_k^2 \sin^2 \theta}.
\end{eqnarray}
The second constraint requires that $x$ is positive which can be seen from Equation \ref{eq:x_substitution}. Since $\cos\ \theta$ may be negative, both positive and negative square root solutions must be considered. Now we can solve Equation \ref{eq:x_substitution}, substituting for $\alpha$ and $v_r$ to obtain an equation for $M_1'$:
\begin{equation}
M_1' = \frac{ \frac{A' x^2}{G} \pm \sqrt{ \frac{A'^2 x^4}{G^2} + 4 (1-\beta) \left( \frac{A' x^2}{G} - \beta^2 \right) M_2' }}{2 (1-\beta)}.
\end{equation}
